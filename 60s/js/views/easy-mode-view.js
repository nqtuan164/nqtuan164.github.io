...
define([
	...
	'underscore',
 	'backbone',
 	'models/easy-mode',
 	'text!templates/easy-mode.html'
], function ($, _, Backbone, EasyMode, EasyModeTemplate) {
	var EasyModeView = Backbone.View.extend({
		el: '#container',
		second: 60,
		interval: null, 
		number: 0,
		...
		render: function() {
			...
			this.$el.html(_.template(EasyModeTemplate, {cells: this.cells}));
		...
		check: function() {
			if(this.temporary > this.number) {
				...
				for(var i = 0; i < _.size(this.chooseArr); i++) {
					...
					//this.cells[idx] = -1;
					$('#cell-' + id).addClass('cell-hidden').removeClass('cell-checked').off();
				}
			...
			if(this.isSuccess == true) {
				...
				FB.getLoginStatus(function(response) {
					FB.ui({
						...
						caption: 'You won!',
						description: 'You finished the Easy mode in ' + sec + ' seconds. You\'re so intelligent!'
					}, function(response) {
